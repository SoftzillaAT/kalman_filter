...
void ParticleFilter::weight_sample(const MeasurementLaserConstPtr &z, std::vector<size_t> used_beams, int start, int end, double *sum) {
...
void ParticleFilter::weighting ( const MeasurementLaserConstPtr &z ) {
    ...
    int bucket_size = (int)ceil(samples.size() / config_.num_threads);
    
    while( idx < samples.size() ) {
        int end = idx + bucket_size - 1;
        if (end >= samples.size())
            end = samples.size() - 1;

        my_threads[i] = std::thread (&ParticleFilter::weight_sample, this, z, used_beams, idx, end, &samples_weight_sum);
        idx += bucket_size;
        i++;
    }

    for(int j = 0; j < config_.num_threads; j++) {
        if (my_threads[j].joinable())
            my_threads[j].join();
    }
...
void ParticleFilter::resample () {
    ...
    int num_resample = samples.size() * config_.resample_rate;
    if (config_.resample_method == SIMPLE_RESAMPLE) {
	std::sort ( samples.begin(),  samples.end(), Sample::greater );
	
        for(int i = 0; i < num_resample; i++) {
            //ROS_INFO("RESAMPLE %d", i);
            SamplePtr &s = samples[samples.size() - i - 1];
	    s->set(samples[i]);
            normal ( s, *s, config_.sigma_static_position, config_.sigma_static_orientation );
        }
    }
    else if (config_.resample_method == LOW_VARIANCE_RESAMPLE) {
      // some basic check
      if (samples.size() > 0) {
	std::sort ( samples.begin(),  samples.end(), Sample::greater ); 
	std::vector<int> resample_vec;
	
	// create floating number between 0 and 1/num_resample
	float r = (1.0 / num_resample) / ((rand() % 100) + 1);
	float c = samples[0]->weight();
	int i = 0;
	
	
	for (int m = 0; i < num_resample; m++) {
	  float U = r + m * (1.0 / num_resample);
	  while (U > c && i < samples.size() - 1) {
	    i++;
	    c = c + samples[i]->weight();
	  }
	  
	  // store index of sample, which will be resampled
	  resample_vec.push_back(i); 
	}
	
	
	// resample choosen samples
	for(int i = 0; i < resample_vec.size(); i++) {
	  int idx = resample_vec[i];
	  SamplePtr &s = samples[i];
	  s->set(samples[idx]);
	  normal ( s, *s, config_.sigma_static_position, config_.sigma_static_orientation );
	}
